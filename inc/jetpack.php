 ...
 * @since Vendeur 1.0
 */
function vendeur_remove_jp_share( $domain ) {
	remove_filter( 'the_content', 'sharing_display',19 );
	remove_filter( 'the_excerpt', 'sharing_display',19 );
}
add_action( 'loop_start', 'vendeur_remove_jp_share' );
 ...
 * @since Vendeur 1.0
 */
function vendeur_remove_jp_related( $domain ) {
...
add_action( 'loop_start', 'vendeur_remove_jp_related' );
 ...
 */
function vendeur_jetpack_sharing_display_text( $text, $service, $id, $args ) {
	if ( class_exists( 'Jetpack_Options' ) && Jetpack_Options::get_option_and_ensure_autoload( 'sharedaddy_disable_resources', '0' ) ) {
		$text = vendeur_svg_icon( 'social-' . $service->get_id() ) . '</span><span class="screen-reader-text">' . $text ;
	}

	return $text;
}
add_filter( 'jetpack_sharing_display_text', 'vendeur_jetpack_sharing_display_text', 4, 10 );
 ...
 * @param array $args Array of sharing service options.
 */
function vendeur_jetpack_sharing_display_classes( $klasses, $service, $id, $args ) {
	if ( class_exists( 'Jetpack_Options' ) && Jetpack_Options::get_option_and_ensure_autoload( 'sharedaddy_disable_resources', '0' ) ) {
...
add_filter( 'jetpack_sharing_display_classes', 'vendeur_jetpack_sharing_display_classes', 4, 10 );
 ...
 *
 */
function vendeur_jetpack_inline_css() {
	...
	wp_add_inline_style( 'vendeur-style', $css  );
}
add_action( 'wp_enqueue_scripts', 'vendeur_jetpack_inline_css', 10 );
