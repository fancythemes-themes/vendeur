<?php
/**
 * Widget API: Vendeur_Widget_Recent_Posts class
 *
 * @package Vendeur
 * @since 1.0.0
 */
 ...
 * @see WP_Widget
 */
class Vendeur_Widget_Recent_Posts extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => esc_html__( "Your site&#8217;s most recent Posts.", 'vendeur'),
			'customize_selective_refresh' => true,
		 );
		parent::__construct('vendeur-recent-posts', esc_html__('Vendeur - Recent Posts', 'vendeur'), $widget_ops);
	...
	public function widget( $args, $instance ) {
		...
		if ( isset($instance['is_slider'] ) && $instance['is_slider'] == 'slider' ) {
			$slider_class = ' posts-slider content-slider';
			$is_slider = true;
...
?>
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'vendeur' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $title; ?>" /></p>

		<p><label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php esc_html_e( 'Category:', 'vendeur' ); ?></label>
			<?php wp_dropdown_categories( 
					array (
						'show_option_all' => esc_html__('All Categories', 'vendeur'),
		...
		<p><label for="<?php echo $this->get_field_id( 'tags' ); ?>"><?php esc_html_e( 'Tags:', 'vendeur' ); ?></label>
		<input class="widefat" id="<?php $this->get_field_id( 'tags' ); ?>" name="<?php echo $this->get_field_name( 'tags' ); ?>" type="text" value="<?php echo $tags; ?>" />
		</p>

		<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php esc_html_e( 'Number of posts to show:', 'vendeur' ); ?></label>
		<input class="tiny-text" id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="number" step="1" min="1" value="<?php echo $number; ?>" size="3" /></p>

		<p><label for="<?php echo $this->get_field_id( 'sort' ); ?>"><?php esc_html_e( 'Sort By:', 'vendeur' ); ?></label>
			<select id="<?php echo $this->get_field_id( 'sort' ); ?>" name="<?php echo $this->get_field_name( 'sort' ); ?>" >
				<option value="date" <?php selected( $sort, 'date' ); ?> > <?php esc_html_e('Date', 'vendeur'); ?> </option>
				<option value="comment_count" <?php selected( $sort, 'comment_count' ); ?> > <?php _e('Comments Number', 'vendeur'); ?> </option>
			</select>
		</p>
		<p>
			<span><?php _e('Presented as', 'vendeur'); ?></span><br>
			...
			<label for="<?php echo $this->get_field_id( 'presentation' ) . '-1'; ?>"><?php esc_html_e( 'Large Thumbnail', 'vendeur'); ?></label><br>
			...
			<label for="<?php echo $this->get_field_id( 'presentation' ) . '-2'; ?>"><?php esc_html_e( 'Small thumbnail', 'vendeur'); ?></label><br>
			...
			<label for="<?php echo $this->get_field_id( 'presentation' ) . '-3'; ?>"><?php esc_html_e( 'Image Overlay', 'vendeur'); ?></label><br>
			...
			<label for="<?php echo $this->get_field_id( 'is_slider' ); ?>"><?php esc_html_e( 'Show as slider', 'vendeur'); ?></label><br> 

			<span class="slider-options">
				<input type="checkbox" id="<?php echo $this->get_field_id( 'slideshow' ); ?>" name="<?php echo $this->get_field_name( 'slideshow' ); ?>" value="slideshow" <?php checked( $slideshow, 'slideshow'); ?> /><label for="<?php echo $this->get_field_id( 'slideshow' ); ?>"><?php esc_html_e('Slideshow', 'vendeur'); ?></label><br>
				<label for="<?php echo $this->get_field_id( 'slideshow_time' ); ?>"><?php esc_html_e( 'Slideshow time:', 'vendeur' ); ?></label><br>
				...
				<?php esc_html_e('in seconds', 'vendeur'); ?><br>
			</span>
		</p>
<?php
	}
...
function Vendeur_widget_recent_posts() {
	return register_widget("Vendeur_Widget_Recent_Posts"); 
}
add_action('widgets_init', 'Vendeur_widget_recent_posts');
