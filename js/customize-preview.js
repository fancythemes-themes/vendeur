...
( function( $ ) {
	...
	var style = $( '#vendeur-color-scheme-css' ),
		api = wp.customize;

	if ( ! style.length ) {
		style = $( 'head' ).append( '<style type="text/css" id="vendeur-color-scheme-css" />' )
		                    .find( '#vendeur-color-scheme-css' );
	}
	...
	function writeCSS(){
		...
		$('#vendeur-preview-style-inline-css').text(cssOutput);
	}
